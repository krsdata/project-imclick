$(function() {
    ...
    if (typeof buildingImagePage !== 'undefined') {
        if ($(".pagination").length > 0) {
            var hrefs = $(".pagination").find("a");
            var params = QueryString.q;
            for (var i = 0; i < hrefs.length; i++) {
                var a = $(hrefs[i]);
                var value = a.attr("href") + "&q=" + params;
                a.attr("href", value);
            }
        }
    }
...
function changeStatusBuilding(id,method)
{
    var status =  $('#'+id).attr('data');
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(response==1)
            {
                $('#'+id).html('Inactive'); 
                $('#'+id).attr('data',response);
                $('#'+id).removeClass('label label-warning status').addClass('label label-warning status');
            ...
            else if(response==2)
            {
                $('#'+id).html('Active'); 
                $('#'+id).attr('data',response);
                $('#'+id).removeClass('label label-success status').addClass('label label-success status');
                 $('#btn'+id).removeAttr('disabled');
            }
            else if(response==3)
            {
