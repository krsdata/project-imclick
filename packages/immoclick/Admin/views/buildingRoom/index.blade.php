                ...
                <div class="box-body table-responsive no-padding " style="overflow:inherit">
                  <table class="table table-hover">
                    ...
                    @if(count($buildingRoom)==0)
                      ...
                      @endif
                     @foreach ($buildingRoom as $key => $building)  
                      <tr>
                        <td>  {{ $building->id }}</td>
                        <td>  {{ $building->Room }} </td>
                        <td>  {{ $building->Stage }} </td>
                        <td>  {{ $building->Width_X }} </td>
                        <td>  {{ $building->Height_Y }} </td>
                        <td>  {{ $building->Floor_type }} </td> 
                         <td> {{ $building->updated_at }} </td>
                        <td> 
                         <a href="{{ route('buildingRoom.edit',$building->id,1)}}?q=1">
                          
                            <i class="fa fa-fw fa-pencil-square-o" title="edit"></i>
                   
                        </a>   
                             {!! Form::open(array('class' => 'form-inline pull-left deletion-form', 'method' => 'DELETE',  'id'=>'deleteForm_'.$building->id, 'route' => array('buildingRoom.destroy', $building->id))) !!}
